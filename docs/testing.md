...
The iOS interaction runner holds temporary display and idle-sleep assertions for its lifetime and has a 30-minute process deadline, followed by bounded artifact export. This prevents laptop sleep from suspending event synthesis and ensures a stuck XCTest process reports failure before the CI job timeout. The assertions end with the runner; system power preferences are unchanged. The local Android app CLI holds the same temporary assertions on macOS, including failure paths. Keep the Mac awake during standalone UI scripts: display sleep can stall simulator accessibility and emulator graphics even while a process remains alive.
